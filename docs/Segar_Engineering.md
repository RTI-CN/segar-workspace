...
3. **Code and Configuration**: Writing business code and CMake configuration
...
- **Protobuf messages**: defined using `.proto`
...
The definition syntax of `srv` and `action` is the same as that of ROS 2, so no further details will be given.
...
|------|------|
| The first parameter | The directory where the type file is located (such as `type_src`) |
| 2nd parameter | Generated code output directory (such as `generate`) |
| The third parameter | The library name generated by type code compilation (such as `example_msg`) |

## 3. Dependency management

### 3.1 Dependency list

Specify in `depend_libs.txt`:
...
```
- `PLATFORM_NAME`: platform identifier (such as `x86_64`, `orin`)
...
Taking `topic_talker` as an example, the original content and comments of its `config/segar.pb.conf` are as follows. Post-deployment path: `topic_example/topic_talker/config/segar.pb.conf`.
...
Taking `topic_talker` as an example, the original content and comments of its `scripts/launch.sh` are as follows. Post-deployment path: `topic_example/topic_talker/scripts/launch.sh`. Run mode: execute after `bash scripts/launch.sh` or `source segar_setup.bash`.
...
topic_talker
```
### 5.5 Preparation before running

`source segar_setup.bash` needs to be loaded before running. For more examples and operation instructions, see [Examples](Segar_Examples.md).
