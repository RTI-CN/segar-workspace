...
# Usage: ./scripts/run_segar_cli_test.sh [output_dir]
...
LOG_FILE="${LOG_DIR}/run_segar_cli_test_${RUN_TS}.log"
mkdir -p "$LOG_DIR"
...
echo "=== segar bag ===" | tee -a "$LOG_FILE"
run_cmd timeout 10 segar bag record -t /topic/chatter -o segar_cli_bag
run_cmd segar bag info segar_cli_bag
run_cmd segar bag play segar_cli_bag
run_cmd rm -rf segar_cli_bag
