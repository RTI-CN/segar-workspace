...
#ifndef SEGAR_EXAMPLES_COMPONENT_TEST_NODE_B_H
#define SEGAR_EXAMPLES_COMPONENT_TEST_NODE_B_H
...
class NodeBComponent
 ...
 public:
  ...
  bool Init() override;
  bool Proc(const std::shared_ptr<IntegrationTestData>& sensor2_msg,
            ...
            const std::shared_ptr<IntegrationTestData>& node_a_msg) override;

 private:
  std::shared_ptr<Writer<IntegrationTestData>> node_b_writer_ = nullptr;
  std::shared_ptr<rti::segar::service::Client<AddTwoInts>> add_two_client_ =
      nullptr;
  uint64_t add_two_seq_ = 0;
  uint64_t sequence_num_;
};
SEGAR_REGISTER_COMPONENT(NodeBComponent)
#endif  // SEGAR_EXAMPLES_COMPONENT_TEST_NODE_B_H
