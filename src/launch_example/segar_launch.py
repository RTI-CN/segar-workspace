...
launch_path = os.getenv('RTI_LAUNCH_PATH') or '/rti'
force_stop_timeout_secs = 3


def resolve_launch_file_from_argv(argv):
    """
    Best-effort resolve launch file from CLI args before argparse.
    This is used by early logger initialization.
    """
    if len(argv) < 3:
        return None
    command = argv[1]
    if command not in ('start', 'stop'):
        return None
    launch_file = argv[2]
    if not launch_file or launch_file.startswith('-'):
        return None
    if os.path.isfile(launch_file):
        return launch_file
    candidate = os.path.join(launch_path, launch_file)
    if os.path.isfile(candidate):
        return candidate
    return None


def get_env_var_from_launch(launch_file, env_key):
    """
    Read env_key value from launch <environment> sections.
    """
    if not launch_file:
        return None
    try:
        ...
        for env in root.findall('environment'):
            for var in list(env):
                if var.tag == env_key and var.text:
...
# file logger
workspace_path = os.getenv('RTI_ENV_WORKROOT')
if not workspace_path:
    launch_file_for_env = resolve_launch_file_from_argv(sys.argv)
    workspace_path = get_env_var_from_launch(
        launch_file_for_env, 'RTI_ENV_WORKROOT')
if not workspace_path:
    workspace_path = '/rti'
...
try:
    ...
    # Final fallback for environments without /rti write permission.
    workspace_path = os.path.join(os.getcwd(), '.segar')
    log_dir_path = os.path.join(workspace_path, 'data/log')
    os.makedirs(log_dir_path, exist_ok=True)
log_name = 'segar_launch.log.INFO.{}.{}'.format(
...
link_log_file = os.path.join(log_dir_path, 'segar_launch.INFO')
try:
...
def start(launch_file=''):
    ...
    for module in root.findall('module'):
        ...
        sched_name = get_param_value(module, 'sched_name', 'SEGAR_DEFAULT')
...
def stop_launch(launch_file):
    ...
    """
    if not launch_file:
        cmd = 'pkill -INT segar_launch'
    else:
        cmd = 'pkill -INT -f ' + launch_file

    os.system(cmd)
    time.sleep(3)
    logger.info('Stop segar launch finished.')
    sys.exit(0)
